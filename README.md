...
| | |
|---|---|
| **Stage** | Daily briefing MVP · NWS forecast weather (when available) · static intelligence previews |
...
**What ships today:** a **Daily Driver Briefing** that synthesizes the MVP stack into shift guidance, **NWS forecast weather** when the public API is reachable (demo fallback when not — not real-time observations), plus static **Events**, **Airport**, and **Demand Zones** previews for rideshare, delivery, Uber Black/SUV, and private drivers. No user accounts or payments yet.
...
Future phases may add TypeScript, a database, and more free/public data APIs — see [docs/DATA_SOURCES.md](docs/DATA_SOURCES.md). Weather uses the free NWS forecast API with no key; demo fallback when unavailable. No paid APIs or secrets in the current MVP.
...
| Phase | Focus | Target |
|---|---|---|
| **Phase 0** *(now)* | Daily briefing + NWS forecast weather + intelligence previews | ✅ Mostly complete |
